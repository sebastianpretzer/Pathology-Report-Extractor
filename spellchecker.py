...
def best(inputted_word, suggestions, word_model=None):
    ...
    def cmp_temp(score1, score2):
        score1 = str(score1)
        score2 = str(score2)
        cmp = (score1 > score2) - (score1 < score2)
        return cmp

    def comparehamm(one, two):
        score1 = hamming_distance(inputted_word, one)
        score2 = hamming_distance(inputted_word, two)
        return cmp_temp(score1, score2)  # lower is better

    def comparefreq(one, two):
        score1 = frequency(one, word_model)
        score2 = frequency(two, word_model)
        return cmp_temp(score2, score1)  # higher is better

    # take the top 10
    freq_sorted = sorted(
        suggestions, key=functools.cmp_to_key(comparefreq))[:10]
    # take the top 10
    hamming_sorted = sorted(
        suggestions, key=functools.cmp_to_key(comparehamm))[:10]
    ...
    def run_main():
        ...
        word_model = train(open('/usr/share/dict/words').read())
